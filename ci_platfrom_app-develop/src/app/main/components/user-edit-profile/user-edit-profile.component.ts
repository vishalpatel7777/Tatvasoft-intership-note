...
export class UserEditProfileComponent implements OnInit, OnDestroy {
  ...
  saveSkill() {
  if (!this.skillList1 || this.skillList1.length === 0) {
    this._toast.warning({
      detail: 'WARNING',
  ...
  const addUserSkillSubscribe = this._commonService.addUserSkill(value).subscribe(
    (data: any) => {
      if (data.result == 1) {
        this._toast.success({ 
          detail: 'SUCCESS', 
          summary: data.data || 'Skills updated successfully',
          duration: APP_CONFIG.toastDuration
        });
        this.closeAddYourSkillModal();
        this.getUserSkill(); // Refresh the skill list
      } else {
        this._toast.error({
          detail: 'ERROR',
          summary: data.message || 'Failed to save skills',
          duration: APP_CONFIG.toastDuration
        });
      }
    },
    (err) => {
      this._toast.error({
        detail: 'ERROR',
        summary: 'Failed to save skills. Please try again later.',
        duration: APP_CONFIG.toastDuration
      });
  ...
  getUserSkill() {
  const userSkillSubscribe = this._commonService.getUserSkill(this.loginUserId).subscribe(
    (data: any) => {
      if (data.result == 1) {
        if (data.data?.length > 0) {
          this.userSkillList = data.data[0].text?.split(',') || [];
        } else {
        ...
        this._toast.error({ 
          detail: 'ERROR', 
          summary: data.message || 'Failed to load skills',
          duration: APP_CONFIG.toastDuration
        });
      }
    },
    (err) => {
      this._toast.error({
        detail: 'ERROR',
        summary: 'Failed to load skills. Please try again later.',
        duration: APP_CONFIG.toastDuration
      });
      console.error('Error loading skills:', err);
    }
  );
  this.unsubscribe.push(userSkillSubscribe);
}


  getCountryList() {
    const countryListSubscribe = this._commonService.countryList().subscribe((data: any) => {
      console.log("get countery list : ", data );
      if (data.result == 1) {
 ...
 onSelectImage(event: any) {
  if (event.target.files && event.target.files[0]) {
    ...
    if (!validTypes.includes(file.type)) {
      this._toast.error({
        detail: 'ERROR',
        summary: 'Only JPEG, PNG, and GIF images are allowed',
        duration: APP_CONFIG.toastDuration
      });
      return;
    }
...
// this.userImage = this._service.getImageUrl(this.editData.userImage);
//   console.log("url" , this.userImage);
// }
...
// user-edit-profile.component.ts
async onSubmit() {
  let imageUrl = '';
  ...
  if (this.userProfileForm.valid) {
    if (this.isFileUpload) {
      try {
        ...
        if (res?.success && res.files && res.files.length > 0) {
          imageUrl = res.files[0]; // e.g., UploadedFiles/MissionImages/<filename>
        ...
        this._toast.error({
          detail: 'ERROR',
          summary: err.message || 'Failed to upload image',
          duration: APP_CONFIG.toastDuration,
        });
        return;
      }
    ...
    // Ensure userImage is a relative path without leading slash
    if (formValue.userImage.startsWith('/')) {
      formValue.userImage = formValue.userImage.substring(1);
    }
    ...
    console.log('Form value before update:', JSON.stringify(formValue, null, 2));
    const userProfileUpdateSubscribe = this._service.loginUserProfileUpdate(formValue).subscribe(
      ...
      (err) => {
        this._toast.error({
          detail: 'ERROR',
          summary: err.message || 'Failed to update profile',
          duration: APP_CONFIG.toastDuration,
        });
        console.error('Profile update error:', err, 'Status:', err.status, 'URL:', err.url);
      }
    );
    ...
    (data: any) => {
      if (data.result == 1) {
        ...
        if (this.editData != undefined) {
          ...
          if (this.editData.userImage) {
            this.userImage = this._service.getImageUrl(this.editData.userImage);
            console.log('Fetched image URL:', this.userImage); // Debug log
          } else {
            this.userImage = ''; // Ensure userImage is empty if no image exists
          }
          this.unsubscribe.push(cityListSubscribe);
        }
      } else {
