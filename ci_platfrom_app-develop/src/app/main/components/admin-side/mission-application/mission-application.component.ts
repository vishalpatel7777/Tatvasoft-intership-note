...
export class MissionApplicationComponent implements OnInit, OnDestroy {
  ...
  fetchMissionApplicationList() {
    const missionApplicationSubscription = this._service.missionApplicationList().subscribe((data: any) => {
      console.log("Mission Application : " , data);
      if (data.result == 1) {
        this.applicationList = data.data;
        console.log("Mission Application : " , data.data);
      }
      else {
...
approveMissionApplication(value: any) {
  ...
  const missionApplicationSubscription = this._service.missionApplicationApprove(payload).subscribe(
    (data: any) => {
      console.log("Mission Application Approve:", data);
      if (data.result === 1) {
