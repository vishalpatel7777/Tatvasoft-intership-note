...
export class MissionService {
  ...
  updateMission(data: Mission) {
    return this.http.put(`${this.apiUrl}${API_ENDPOINTS.MISSION.UPDATE}`, data)
  }
  ...
  // Transform the data to match the expected DeleteMissionApplicationModel format
  const deleteModel = {
    ApplicationId: data.id || data.applicationId || data.Id  // Handle different property naming conventions
  };
  
  return this.http.post(`${this.apiUrl}${API_ENDPOINTS.MISSION.APPLICATION_DELETE}`, deleteModel);
}
