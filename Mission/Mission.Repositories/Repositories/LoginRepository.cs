...
namespace Mission.Repositories
{
    public class LoginRepository(MissionDbContext cIDbContext) : ILoginRepository
    {
        ...
        public async Task<UserProfileDetailResponseModel> GetUserProfileDetailById(int userId)
        {
            var userDetails = await _cIDbContext.UserDetails
                .Where(ud => ud.UserId == userId && !ud.IsDeleted)
                .Select(ud => new UserProfileDetailResponseModel
                {
                    UserId = ud.UserId,
                    Name = ud.Name,
                    Surname = ud.Surname,
                    ...
                    CityId = ud.CityId,
                    Avilability = ud.Availability,
                    LinkdInUrl = ud.LinkedInUrl,
